...
管理者画面の「タイル追加設定」でHTTPSの画像タイルURL（`{z}/{x}/{y}`）、出典、ズーム範囲、不透明度、表示順、有効状態を設定できます。
起動時のマイグレーション11で専用テーブルを自動追加します。既存の背景地図タイル設定はそのまま利用できます。

ユーザーは新規レイヤ作成モーダルの「重ね合わせタイル」で登録・解除します。変更は自動保存されます。
通常地図・モバイル・別画面の表示チェック状態はブラウザ内でアカウントごとに保存し、削除・無効化・登録解除されたタイルの保存値を整理します。
一時共有画面には共有URL作成者、位置共有マップにはマップを作成した管理者の最新設定をページ読込時に反映します。
出典のHTTP・HTTPSリンクは別タブで開きます。画像の読込失敗は対象地域にデータがない場合もあるため、URL誤りとは断定せずデバッグログに記録します。


ライブマップの重ね合わせタイルは初期状態で非表示です。共有URLに `?is_check_overlay=true` を付けると登録済みのタイルをすべて表示します。
`false`、未指定、不正な値は非表示となり、閲覧者はコントロールから個別に表示できます。管理画面の「重ね合わせタイルを初期表示する」でコピー・開くURLを切り替えられます。この選択はDBに保存せず、パスワード認証後もURLの指定を引き継ぎます。
