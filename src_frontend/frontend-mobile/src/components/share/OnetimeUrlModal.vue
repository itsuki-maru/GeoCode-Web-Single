...
const invalidateOneTimeMap = async (): Promise<void> => {
  try {
    await apiClient.delete(invalidateOntimeMapUrl + `${props.uuid}`);
    emit("invalidated");
...
<template>
  <BaseModal :isOpen="isOpen" :zIndex="15" :close-on-overlay-click="false" @close="emit('close')">
    <h2 class="modal-h2">メッセージ</h2>
    <div class="input-text-zone" v-if="isHttps">
      <p><strong>共有マップを作成しました。</strong></p>
      <p v-if="expiration" class="expiration-text">有効期限: {{ formatExpiration(expiration) }}</p>
    ...
    <div class="input-text-zone" v-else>
      <p><strong>共有マップを作成しました。</strong></p>
      <p v-if="expiration" class="expiration-text">有効期限: {{ formatExpiration(expiration) }}</p>
      <pre :id="uuid"><code :id="uuid">{{ url }}</code></pre>
      <pre><code :id="uuid" @click="selectTextOrClipboardCopy(`${uuid}`)">{{ url }}</code></pre>
    </div>
    <div class="btn-zone">
      <button id="message-close-btn" @click="emit('close')" class="btn-standard">閉じる</button>
      <button id="message-close-btn" @click="invalidateOneTimeMap()" class="btn-standard">
        共有停止
      </button>
    </div>
  </BaseModal>
