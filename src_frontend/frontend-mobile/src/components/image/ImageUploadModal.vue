...
const emit = defineEmits<{
  close: [];
  uploaded: [markdownLink: string, uniqueFileName: string];
  message: [text: string];
...
const uploadImage = async (): Promise<void> => {
  if (isImageSendNow.value) return;
  ...
  try {
    ...
    } else if (isPDF(uniqueFileName)) {
      imageUrlMarkdown = `[${selectedFileName.value}](${baseUrl}/static/images/${uniqueFileName})`;
    } else {
      imageUrlMarkdown = `![${selectedFileName.value}](${baseUrl}/static/images/${uniqueFileName})`;
    }

    emit('uploaded', imageUrlMarkdown, uniqueFileName);
    imageStore.initList();
    imageClear();
  } catch (error) {
    console.error(error);
  ...
  <BaseModal :isOpen="isOpen" @close="emit('close')">
    ...
    <div class="upload-panel">
      <label for="image1" class="file-picker-card">
        ...
        <span class="file-picker-subtitle">タップして画像・動画・PDFを追加</span>
      </label>
      <input
        ref="fileInputRef"
        type="file"
      ...
      <div class="action-row">
        <button type="submit" class="btn-file-upload" :disabled="!isUploadReady" @click.prevent="uploadImage()">
          {{ isImageSendNow ? 'アップロード中...' : 'アップロード' }}
        </button>
        <button type="button" class="btn-secondary" :disabled="!selectedFileName" @click.prevent="imageClear()">
          選択をクリア
        </button>
      </div>
    </div>
    ...
    <div class="btn-zone">
      <button @click.prevent="emit('close')" class="btn-standard">閉じる</button>
    </div>
  </BaseModal>
</template>
