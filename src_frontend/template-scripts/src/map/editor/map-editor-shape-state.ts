...
const mapEditorProfile = Object.freeze({
  ...
  isShapeVisibleForMeasurement: (layer) =>
  ...
  shouldSuppressShapeLabelClick: () =>
    editorEntryProfile.isMobile ? false : isShapeLabelClickSuppressed(),
});
...
const SHAPE_VERTEX_ADD_TOLERANCE_PX = editorEntryProfile.isMobile ? 24 : 14;
const SHAPE_VERTEX_MIN_DISTANCE_PX = editorEntryProfile.isMobile ? 16 : 10;
...
function beginDrawing(mode) {
  ...
  clearDrawPreview();
  updateShapeDrawingState();
  const interactionVerb = editorEntryProfile.interactionVerb;
  if (mode === "rectangle") {
    setDrawStatus(`図形描画: 矩形の1点目を${interactionVerb}してください。`);
  } else if (mode === "circle") {
    setDrawStatus(`図形描画: 円の中心を${interactionVerb}してください。`);
  } else if (mode === "delete") {
    setDrawStatus(`図形描画: 削除したい図形を${interactionVerb}してください。`);
  } else if (mode === "polyline") {
    setDrawStatus(`図形描画: 線の頂点を${interactionVerb}し、完了を押してください。`);
  } else {
    setDrawStatus(`図形描画: 面の頂点を${interactionVerb}し、完了を押してください。`);
  }
  updateShapesInteractionStyle();
  const drawControl = document.getElementById("draw-control");
  if (drawControl) {
