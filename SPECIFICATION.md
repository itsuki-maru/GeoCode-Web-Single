...
既存の管理者画面「タイル追加設定」で、高解像度降水ナウキャストの最新解析画像を登録できる。

SQLiteの既存タイル定義・選択情報を利用し、DB項目やマイグレーションは追加しない。雨雲の取得・自動更新にはインターネット接続が必要となる。
...
| 表示名        | 雨雲の動き（気象庁）                                     |
| 画像タイルURL | 下記参照                                                 |
| 出典          | `<a href="https://www.jma.go.jp/bosai/nowc/">気象庁</a>` |
| 最小ズーム    | 4                                                        |
| 最大ズーム    | 18（10より上は拡大表示）                                 |
| 不透明度      | 0.7                                                      |
| 表示順・有効  | 既存のタイルと同様に設定                                 |
...
`latest` は GeoCode-Web 専用の識別記法である。この URL 自体を気象庁に送信せず、前後の空白を除いた完全一致で判定する。クエリ、別ホスト、別要素、日時固定の URL は通常の XYZ タイルとして扱う。

登録後は既存のタイル選択・公開設定を利用する。編集用・閲覧専用・ライブ地図が同じ実装を利用する。

#### 表示と更新

- `targetTimes_N1.json` から `hrpns` があり、`basetime = validtime` の最新解析を選択。配信時刻は UTC、地図上の対象時刻は日本時間。
- 表示中は通常300秒（5分）間隔で確認し、対象時刻が変わった場合のみ画像 URL を更新。同一ページの取得は共有。取得周期は配信タイミングと同期していないため、新しいデータの反映まで最大約5分かかる場合がある。
- 非表示、タブ非表示、削除、地図破棄で定期取得を止める。最後の利用者がいなくなると時刻取得通信も中止。再表示時に再確認。
- 通信は15秒で打ち切り、失敗時は2分、4分、最大5分間隔で再試行。
- 対象時刻から15分以上経過した画像は非表示にする。これは本アプリの鮮度基準であり、気象庁の配信保証ではない。通信とは別に、表示中は最大約1分の間隔で画像の古さを確認。
- 取得失敗・対象時刻・画像の部分的な取得失敗は `nowcast-status` に表示する。表示チェックは維持するため、復旧後は自動的に再表示する。
- 配信ズーム4・6・8・10に合わせてタイルの格子全体を調整。奇数ズームと10より上は拡大表示。管理者の最大ズームは表示上限として維持。

#### ステータスの画面別表示

ステータスには表示名、対象時刻・取得状況、気象庁の雨雲・凡例へのリンクを表示する。「透明部分には欠測・対象外を含みます。」という説明文は表示しない。

| 画面 | 配置・表示切替 |
| --- | --- |
| `map.html` | 左下のモード説明より上に配置する。図形ツールはステータスより手前に表示する |
| `map-mobile.html` | レイヤー一覧コントロールの直下に、余白を設けた独立した枠として配置する。一覧とは結合せず、「地図だけを表示／機能を表示」に連動する |
| `map-anather.html` | 左下の「閉じる」ボタンより上に配置し、画面の高さに応じた余白を設ける |
| `temporary-map.html` | 左下に表示する |
| `temporary-map-mobile.html` | 左下に配置し、「地図だけを表示／機能を表示」に連動する |
| `live-map.html` | PCでは左下に表示し、幅700px以下では非表示にする |

ステータスだけを非表示にしても雨雲タイルの表示状態や更新は変更しない。UI非表示中に雨雲レイヤーを再追加した場合も、ステータスは現在のUI表示状態を引き継ぐ。

#### 対応範囲

降水強度の最新解析 PNG (`hrpns`) のみ対応。予報時刻の選択、過去画像再生、雷・竜巻は対象外とする。凡例は地図上の気象庁リンクから確認できる。

欠測域を示す別の GeoJSON (`hrpns_nd`) は重ねていない。透明部分には無降水だけでなく欠測・対象外が含まれる。詳細は気象庁の表示を確認する。

#### 保守

`src_frontend/template-scripts/src/map/common/nowcast-time.ts` が判定と時刻取得、`nowcast-layer.ts` が描画・状態表示を担当。`tile-overlays.ts` が登録・削除を管理。

気象庁への JSON 取得には `src/middleware/security.rs` の CSP `connect-src` 許可を使用。リバースプロキシで別途 CSP を設定している場合も `https://www.jma.go.jp` の接続許可が必要。タイル画像は既存の `img-src https:` を使用。

気象庁サイトが使用する公開ファイルに依存。URL、時刻一覧の形式、配信ズーム、CORS の変更時には再確認する。Leaflet のタイル格子調整には `_clampZoom` を利用しているため、Leaflet 更新時は実ライブラリによるズームテストを実行する。

##### 検証コマンド

```shell
cd src_frontend/template-scripts
npm test
npm run build
```
...
```shell
cargo test --lib middleware::security::tests
```

##### 参照

- [気象庁情報カタログ](https://ds.data.jma.go.jp/suishin/cgi-bin/catalogue/make_product_page.cgi?id=Nowcast)
- [時刻一覧](https://www.jma.go.jp/bosai/jmatile/data/nowc/targetTimes_N1.json)
- [雨雲の動き](https://www.jma.go.jp/bosai/nowc/)
- [気象庁ホームページの利用条件](https://www.jma.go.jp/jma/kishou/info/coment.html)
