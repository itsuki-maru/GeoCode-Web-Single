...
Rust 側のユニットテストと統合テストを、`Cargo.lock` に固定された依存関係で実行する。これにより、バックエンド処理、ルーティング補助、モデル変換など、既存テストで守られている挙動が壊れていないことを確認する。SQLx のクエリ検証は環境変数や SQLite DB の状態に依存するため、ローカルで失敗する場合は `.env` と SQLite マイグレーション状態も確認する。

Rust の handler テストは `tests/handler_sqlite.rs` でテスト用 SQLite を使って実行する。外部 DB サーバや追加サービスは不要で、CI 上は通常の `cargo test --locked` に含まれる。`src/lib.rs` にある軽量ハンドラは SQLite を使わないため、`tests/lib_handlers.rs` で個別に確認する。タイルプロキシの外部上流サーバ通信は CI テスト対象外とし、座標検証や URL 組み立てなど外部通信を伴わない単体テストのみ維持する。
...
1. Settings > Actions > General
   - Actions permissions は `Allow itsuki-maru, and select non-itsuki-maru, actions and reusable workflows` を選択し、CI / Release で使用する外部 Actions だけを許可する
   - `Allow specified actions and reusable workflows` の入力欄には、改行区切りではなくカンマ区切りで以下を 1 行に指定する

```txt
actions/checkout@v4, actions/setup-node@v4, dtolnay/rust-toolchain@stable, swatinem/rust-cache@v2, actions/upload-artifact@v4, actions/download-artifact@v4, softprops/action-gh-release@v2
```
